...
export const toggleVariants = cva(
  'inline-flex b-solid items-center justify-center rounded-md text-sm font-medium transition-colors transition-property-[box-shadow,color,background-color] hover:(bg-muted text-muted-foreground) focus-visible:(outline-none ring-1.5 ring-ring) disabled:(pointer-events-none opacity-50) data-[pressed]:(bg-accent text-accent-foreground) transition-shadow',
  {
    variants: {
      variant: {
        default: 'bg-transparent',
        outline:
          'border border-solid border-input bg-transparent shadow-sm hover:(bg-accent text-accent-foreground)'
      },
      size: {
        default: 'h-9 px-3',
        sm: 'h-8 px-2',
